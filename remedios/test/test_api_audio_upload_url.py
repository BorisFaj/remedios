...
class _DummyObjectStorage:
    models = types.SimpleNamespace(
        CreatePreauthenticatedRequestDetails=_DummyParDetails
    )
...
class DummyClient:
    ...
    def create_preauthenticated_request(self, namespace, bucket, details):
        ...
        return types.SimpleNamespace(
            data=types.SimpleNamespace(access_uri="/p/test-par")
        )
...
def test_audio_upload_url_requires_auth(client):
    resp = client.post(
        "/internal/audio_upload_url", json={"job_id": 1, "audio_id": "a1"}
    )
